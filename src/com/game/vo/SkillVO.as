...
import utils.StringUtil;
	...
	<skill id="1" name="天雷诀" type1="1" type2="1" apTarget="1" apType="0"
        ...
        public static function getStrongAttr(skillid:int):String
        {
            if(skillid==0) return "";
            var skillvo:SkillVO = ConfigLocator.getInstance().skillDatas.get(Math.floor(skillid/100));
            if(!skillvo){
                return "";
            }
            var arr:Array = skillvo.getTip(Math.floor(skillid)%100);
            var result:String = HtmlUtil.fontSizeAndColor( skillvo.name+"  "+Math.floor(skillid)%100 + LangTxt.SUFFIX_LEVEL+"<br>",GoodsVO.TEXTCOLOR[skillvo.degree-1],32)
            +HtmlUtil.color(LangArrayTxt.SKIL_VIEW_4[skillvo.type]+"<br>", "#FFCC99")
            +HtmlUtil.color(LangTxt.SKILL_COOL_DOWN.replace("{0}",skillvo.datas.get((Math.floor(skillid)%100))[3]) + "<br>", "#fefefe")
            +HtmlUtil.color(arr[1]+ "<br>", "#fefefe")
            +HtmlUtil.color(arr[2]+ "<br>", "#FFFF33");
            return result;
        }
        ...
        public static const HeartGhostModel_skillID:uint = 97;
        public static function createTootips(skillid:int,type:int = 0,runeLevel:int = 0,stage:uint = 0):String
        {
            ...
            var skillLevel:uint = Math.floor(skillid)%100;
            var arr:Array = skillvo.getTip(skillLevel,stage);
            var arr1:Array = skillvo.getTip(runeLevel);
            var result:String = "";
            if(type==0)
            {
                result = HtmlUtil.fontSizeAndColor( skillvo.name+"  "+ (skillid)%100 + LangTxt.SUFFIX_LEVEL+"<br>",GoodsVO.TEXTCOLOR[skillvo.degree-1],32);
            }else{
                result = HtmlUtil.fontSizeAndColor( skillvo.name+"  "+ (skillid)%100 + LangTxt.SUFFIX_LEVEL+"<br>",GoodsVO.TEXTCOLOR[skillvo.degree-1],30);
            }
            if(Math.floor(skillid / 100) == HeartGhostModel_skillID)
            {
                result += HtmlUtil.color(arr[1]+ "<br>", "#fefefe");
            }
            else
            {
                result +=HtmlUtil.color(LangArrayTxt.SKIL_VIEW_4[skillvo.type]+"<br>", "#FFCC99")
                +HtmlUtil.color(LangTxt.SKILL_COOL_DOWN.replace("{0}",skillvo.datas.get((Math.floor(skillid)%100))[3])+ "<br>", "#fefefe")
                +HtmlUtil.color(arr[1]+ "<br>", "#fefefe");
            }

            result += HtmlUtil.color(arr[2]+ "<br>", "#FFFF33");

            if(skillvo.apType!=1)
            {
                result += ""//HtmlUtil.color("当前技能无法开启技能符文<br>","#ff0000");
            }else{
                if(runeLevel!=0)
                {
                    result +=HtmlUtil.color("技能符文  " + runeLevel + "阶<br>", "#FF99CC")
                    +HtmlUtil.color(arr1[0]+ "<br>", "#fefefe");
                }
            }
            return result;
        }
