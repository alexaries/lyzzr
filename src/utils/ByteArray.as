...
public class ByteArray extends Byte {
    ...
    var hasTextEncoder:Boolean=false;
    public function ByteArray():void {
        __JS__("this.___resizeBuffers(this._allocated_)");
        hasTextEncoder= __JS__("(typeof TextEncoder !== 'undefined');");
    ...
    override public function ensureWrite(lengthToEnsure:int):void {
        if (this.__length < lengthToEnsure) length = lengthToEnsure;
    }
    ...
    public function readBytes(bytes:ByteArray, offset:int = 0, length:int = 0):void {
        ...
        if (length == 0) length = this.__length - this._position_;
    ...
    public function readUnsignedInt():int {
        ...
        this._position_ += 4;
        return  (uInt);//add by ch.ji 解决读取整数时读到负整数的问题
    }
    ...
    public function __writeUTFBytes(value:String):void {
        // utf8-decode
        value = value + "";
    ...
    public function __readUTFBytes(len:int = -1):String {
        var r = "", max = this.position + len;
        // utf8-encode
        while (this.position < max) {
            var c = _data_.getUint8(this.position++);
            if (c < 0x80) {
                if (c == 0) break; // EOF
                r += String.fromCharCode(c);
            } else if (c < 0xE0) {
                r += String.fromCharCode(((c & 0x3F) << 6) |(_data_.getUint8(this.position++) & 0x7F));
            } else if (c < 0xF0) {
                var c2 = _data_.getUint8(this.position++);
                r += String.fromCharCode(((c & 0x1F) << 12) |((c2 & 0x7F) << 6) |(_data_.getUint8(this.position++) & 0x7F));
            } else {
                var c2 = _data_.getUint8(this.position++);
                var c3 = _data_.getUint8(this.position++);
                r += String.fromCharCode(((c & 0x0F) << 18) |((c2 & 0x7F) << 12) |((c3 << 6) & 0x7F) |(_data_.getUint8(this.position++) & 0x7F));
            }
        }
        return r;
    }
    override public function writeUTFBytes(value:String):void {
        // utf8-decode
        if(!hasTextEncoder){
            __writeUTFBytes(value);
            return;
        }
        var uint8array:*= __JS__(" new TextEncoder('utf-8').encode(value + '');");
        var len:int=uint8array.length;
        ensureWrite(this._position_ + len);
        __JS__("this._byteView_.set(uint8array.subarray (0, len), this._position_)");
        this._position_ += len;
        uint8array=null;
    }
    override public function readUTFBytes(len:int = -1):String {
        if(!hasTextEncoder){
            return __readUTFBytes(len);
        }
        var str:String="";
        __JS__("str= new TextDecoder('utf-8').decode(this._byteView_.subarray(this._position_, this._position_ + len ))");
        this._position_ += len;
    ...
    public function writeInt(value:int):void {
        ensureWrite (this._position_ + 4)
        _data_.setInt32 (this._position_, value, this._littleEndian_);
        this._position_ += 4;
    }
    ...
    public function uncompress(algorithm:String = "zlib"):void {
        ...
        _allocated_ = __length = _byteView_.byteLength;
        _position_ = 0;
    }
    ...
    public function compress(algorithm:String = "zlib"):void {
        ...
        __JS__("this._data_ = new DataView(this._byteView_.buffer);");
        _position_ = _allocated_ = __length = _byteView_.byteLength;
    }
    ...
    override public function get endian():String {
        return _littleEndian_ ? Endian.LITTLE_ENDIAN : Endian.BIG_ENDIAN;
    }


    override public function set endian(endianStr:String):void {
        _littleEndian_ = (endianStr == Endian.LITTLE_ENDIAN);
    }
